...
public class Parser {
     ...
     */
    private func valueLike() -> ConstantNode {
        var quantity: ConstantNode?
        ...
        case let .constant(.string(value)):
            quantity = ConstantNode.string(value)
            eat(.constant(.string(value)))
            
        case let .constant(.decimal(value)):
            quantity = ConstantNode.decimal(value)
            eat(.constant(.decimal(value)))
        ...
        case let .constant(.integer(value)):
            ...
            if currentToken == .slash {
                ...
                quantity = ConstantNode.fractional((numerator, denominator))
                eat(.constant(.integer(denominator)))
            } else {
                quantity = ConstantNode.integer(value)
            }
        case .pipe:
            fatalError("Shit")
        ...
        if quantity == nil {
            quantity = ConstantNode.integer(1)
        }
     ...
     */
    private func metadataValue() -> String {
        guard case let .constant(.string(value)) = currentToken else {
            fatalError("String expected, got \(currentToken)")
        }
